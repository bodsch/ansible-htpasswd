...
def local_facts(host):
    ...
    """
    return (
        host.ansible("setup").get("ansible_facts").get("ansible_local").get("htpasswd")
    )


def test_credentials_path(host, get_vars):
    """ """
    credentials_path = get_vars.get("htpasswd_credentials_path", None)

    print(credentials_path)

    if credentials_path:
        directory = host.file(credentials_path)
        assert directory.is_directory


def test_credentials_files(host, get_vars):
    """ """
    credential_files = get_vars.get("htpasswd_credentials", None)

    pp_json(credential_files)

    credential_files = [x.get("path") for x in credential_files]
